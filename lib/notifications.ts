...
/**
 * "A couple of days before" the promised renewal, per the sign-up copy.
 * Named so it's one place to change, and so the "skip if already inside the
 * lead window" check below reads as intentional rather than a magic number.
 */
export const RENEWAL_REMINDER_LEAD_DAYS = 2;
 ...
 * parseable renewal date, or the renewal is already closer than the lead
 * time (rolling forward can't put a reminder in the past, and firing it
 * "immediately" would misrepresent the promise of a heads-up before the
 * charge).
 */
const reminderTimingFor = (subscription: Subscription): ReminderTiming | null => {
    const renewalDate = nextRenewalDate(subscription.renewalDate, subscription.billing);
    if (!renewalDate) return null;

    const reminderDate = renewalDate
        .subtract(RENEWAL_REMINDER_LEAD_DAYS, 'day')
 ...
 * to be called on every subscription-store change (see
 * lib/useRenewalReminders.ts) as well as on app start, so cancel-then-recreate
 * is deliberate: it's the only way to avoid piling up duplicates across
 * however many times this runs in a session.
 */
export const scheduleRenewalReminders = async (subscriptions: Subscription[]): Promise<void> => {
    ...
    const candidates = subscriptions
        ...
        .map((subscription) => ({ subscription, timing: reminderTimingFor(subscription) }))
        .filter(
